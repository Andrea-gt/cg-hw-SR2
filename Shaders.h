...
Color fragmentShader(Fragment& fragment) {
    fragment.intensity-=0.25f;
    fragment.intensity = glm::clamp(fragment.intensity, 0.0f, 1.0f);
...
Color fragmentShaderEarth(Fragment& fragment) {
    ...
    float bloomIntensity = abs(fragment.intensity) * -1.0; // Adjust the multiplier as needed
    bloomIntensity = (bloomIntensity > -0.5f)? -0.5f: bloomIntensity;
    ...
    fragment.intensity = glm::clamp(fragment.intensity, 0.0f, 1.0f);
    fragment.color = fragmentColor * (1.0f - fragment.intensity);
